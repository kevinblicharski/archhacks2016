        ...
        storage.loadMedList(session, function (medList) {
            ...
            if (medList.data.medications.length === 0) {
                ...
                speechOutput += 'ScoreKeeper, '
                    + 'you have ' + medList.data.medications.length + ' player';
                if (medList.data.medications.length > 1) {
                    speechOutput += 's';
                }
                speechOutput += ' in the game. You can give a player points, add another player, reset all medications or exit. Which would you like?';
